...
## Phase 4 Technical Specifications (Concurrency Integration)

* **Engine Upgrade:** Wrapped the Segregated Fits architecture in a thread-safe control flow using POSIX Mutexes (`pthread_mutex_t`), turning the single-threaded allocator into a concurrent memory engine capable of serving multithreaded environments.
* **The Sandbox Architecture:** Engineered the boot sequence to interface correctly with a simulated hardware layer (`mem_init_sandbox()`). This isolates `yaxmalloc` from the host OS, allocating a massive contiguous byte array (fake RAM) before formatting the initial `mm_init()` boundary tags, preventing system-level Segmentation Faults during development.
* **Race Condition Mitigation:** Forced deterministic testing using a massive execution load (1,000,000 continuous allocations across 4 detached worker threads). This overwhelmed the OS scheduler's quantum, proving that simultaneous writes to the `seg_lists` explicit pointers cause immediate catastrophic failure, establishing the absolute necessity for mutual exclusion.
* **Flawless Critical Sections:** Installed strict, zero-latency locking logic. The engine checks for null allocations *before* requesting the lock to save CPU cycles. Crucially, the control flow guarantees that the lock is released on *every single exit path* (including internal `extend_heap` failures), entirely eliminating the risk of Deadlocks.
* **Thread Lifecycle Management:** Integrated an architectural understanding of Detached vs. Joinable threads. Unlike `pthread_join`, which creates a fatal block on the parent process, detached threads instantly reclaim their own memory upon termination. This prevents Zombie thread accumulation, preparing the engine for high-throughput, low-latency environments where thousands of threads execute and self-destruct rapidly.